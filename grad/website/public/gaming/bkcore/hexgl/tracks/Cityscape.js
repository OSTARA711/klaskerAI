// Path: grad/website/static/gaming/bkcore/hexgl/tracks/Cityscape.js
...
bkcore.hexgl.tracks.Cityscape = {
	...
	load: function(opts, quality)
	{
		...
		/*
		 * HIGH quality is now the only rendering profile.
		 *
		 * The former LOW/MID asset path has been removed. The full
		 * resolution textures, normal maps and specular maps are always
		 * loaded.
		 */
		this.lib.load({
			textures: {
	...
	{
		/*
		 * HIGH quality is hardwired.
		 *
		 * Full-resolution diffuse, specular and normal maps are used
		 * directly. The obsolete MeshBasicMaterial path has been removed.
		 */
	...
	buildScenes: function(ctx, quality)
	{
		...
		/*
		 * Shadow-map rendering has been removed from the game.
		 *
		 * The directional light remains because the materials still use
		 * lighting, but the expensive shadow-map pass is unnecessary.
		 */
		scene.add( sun );
		...
		var fxParams = {
			...
			useParticles: true,
			textureCloud: this.lib.get("textures", "cloud"),
			textureSpark: this.lib.get("textures", "spark")
		};
